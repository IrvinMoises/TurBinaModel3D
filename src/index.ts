...
/**/
/**/
function rotateTurbine(obj: any, af: number, totalIterations: number) {
  // Calcular el centro del objeto
  let center = TurboCenter(obj);
  ...
  function rotateStep() {
    ...
    if (currentIteration <= totalIterations) {
      setTimeout(rotateStep, 8); // Llamar a la función nuevamente después de 100ms
    }
  }

  rotateStep();
}
function TurboCenter(obj: any): Point3D {
  let sumX = 0;
  let sumY = 0;
  let sumZ = 0;
  ...
  const numVertices = 4285; // Número total de vértices del objeto
...
document.getElementById('pza1Izq').addEventListener('click', ()=>{rotateTurbine(obj,-20,6000)}, false);
